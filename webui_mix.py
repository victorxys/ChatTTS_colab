...
def generate_seeds(num_seeds, texts, tq):
    ...
    for _ in tq(range(num_seeds), desc=f"随机音色生成中..."):
        ...
        filename = generate_audio_for_seed(chat, seed, texts, 1, 1, "[oral_0][laugh_0][break_7]", None, 0.3, 0.7, 20)
    ...
    with gr.Tab("长音频生成"):
        with gr.Row():
            with gr.Column():
                ...
                section_input = gr.Textbox(label=f"可以输入章节，便于按章节存储，建议使用英文或数字便于创建相应保存目录）", lines=1,
